    ...
    return (
        <section className="py-24 bg-[#FDFCFB] dark:bg-slate-950 navy:bg-[#020D1A] transition-colors duration-500">
            <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col lg:flex-row items-center gap-20">
                ...
                >
                    <h2 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white navy:text-indigo-50 transition-colors duration-500">
                        Become A <span className="text-primary italic">Seller</span>
                    </h2>
                    <p className="text-lg text-slate-600 dark:text-slate-400 navy:text-indigo-300/70 leading-relaxed transition-colors duration-500">
                    ...
                    <div className="flex flex-wrap items-center gap-6">
                        ...
                        <button className="text-slate-600 dark:text-slate-200 navy:text-indigo-300 font-bold hover:text-primary transition-colors">
                            How to Start →
                        </button>
                    </div>
                </motion.div>
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-6 w-full">
                    {benefits.map((benefit, idx) => (
                        <motion.div
                            ...
                            className="flex items-center gap-6 p-8 bg-[#F9F7F5] dark:bg-slate-900 navy:bg-[#051933] border border-slate-100 dark:border-slate-800 navy:border-indigo-900/30 hover:shadow-xl dark:shadow-slate-950/50 navy:shadow-indigo-950/50 transition-all duration-300"
                        >
                            ...
                            <div>
                                <h4 className="text-3xl font-bold text-slate-900 dark:text-white navy:text-indigo-50 transition-colors duration-500">{benefit.label}</h4>
                                <p className="text-slate-500 dark:text-slate-400 navy:text-indigo-300/70 font-medium transition-colors duration-500">{benefit.desc}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
